...
export default function EditEducationProgramPage() {
  ...
  useEffect(() => {
    const fetchProgram = async () => {
      ...
      try {
        const token = localStorage.getItem('authToken');
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/education/programs/${id}`, {
          headers: {
        ...
        if (response.ok) {
          ...
          const applicationDeadlines = {
            fall: program.applicationDeadlines?.fall ? new Date(program.applicationDeadlines.fall) : null,
            spring: program.applicationDeadlines?.spring ? new Date(program.applicationDeadlines.spring) : null,
            summer: program.applicationDeadlines?.summer ? new Date(program.applicationDeadlines.summer) : null,
          };
          ...
          setFormData({
            ...
            applicationDeadlines,
            admissionRequirements: {
              academicRequirements: program.admissionRequirements?.academicRequirements || [],
              languageRequirements: program.admissionRequirements?.languageRequirements || [],
              documentsRequired: program.admissionRequirements?.documentsRequired || [],
              additionalRequirements: program.admissionRequirements?.additionalRequirements || []
            },
            images: program.images || [],
            tags: program.tags || [],
            institution: {
              name: program.institution?.name || '',
              website: program.institution?.website || '',
              logo: program.institution?.logo || ''
            },
            location: {
              city: program.location?.city || '',
              address: program.location?.address || '',
              campus: program.location?.campus || ''
            },
            language: {
              instruction: program.language?.instruction || 'English',
              requirements: program.language?.requirements || ''
            },
            contactInfo: {
              email: program.contactInfo?.email || '',
              phone: program.contactInfo?.phone || '',
              website: program.contactInfo?.website || '',
              admissionsOffice: program.contactInfo?.admissionsOffice || ''
            }
          });
        } else {
          toast({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: 'Success',
          description: 'Education program updated successfully!',
          variant: 'success'
        });
        router.push('/admin/education');
      } else {
        ...
        toast({
          title: 'Error',
          description: error.message || 'Failed to update education program',
          variant: 'error'
        });
      }
    } catch (error) {
      toast({
        title: 'Error',
        description: 'Failed to update education program',
        variant: 'error'
      });
    } finally {
      setLoading(false);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-gray-600">Loading education program...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Edit Education Program</h1>
          <p className="text-gray-600">Update the education program details</p>
        </div>
